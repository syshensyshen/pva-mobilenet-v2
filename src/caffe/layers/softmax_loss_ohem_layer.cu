...
#ifdef SYSHEN_DEBUG
    LOG(INFO) << "syshen bottom size is: " << bottom.size();
    LOG(INFO) << "syshen top size is: " << top.size();
    for (int i=0; i<bottom.size(); i++) {
        vector<int> shape = bottom[i]->shape();
        for (int s = 0; s<shape.size(); s++) {
             LOG(INFO) << "syshen bottom index " << i << " shape index "<< s << " is " << shape[s];
        }
    }
    for (int i=0; i<top.size(); i++) {
        if (0 == i) {
	    LOG(INFO) << "syshen test top index 0!";
	}
        vector<int> shape = top[i]->shape();
        for (int s = 0; s<shape.size(); s++) {
             LOG(INFO) << "syshen top index " << i << " shape index " << s  << " is " << shape[s];
        }
    }
#endif
