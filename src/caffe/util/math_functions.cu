...
void caffe_gpu_square<float>(const int N, const float* a, float* y) {
  ...
  square_kernel<<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS>>>(N, a, y);
...
template <>
void caffe_gpu_l2norm<float>(const int n, const float* x, float* y) {
	CUBLAS_CHECK(cublasSnrm2(Caffe::cublas_handle(), n, x, 1, y));
}

template<> 
void caffe_gpu_l2norm<double>(const int n, const double *x, double *y) {
